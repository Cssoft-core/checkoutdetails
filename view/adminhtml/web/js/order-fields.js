define([
    ...
    return Component.extend({
        defaults: {
            template: 'CSSoft_CheckoutDetails/order/view',
            listens: {
                '${ $.provider }:cssoftCheckoutFields': 'fieldsChanged'
            }
        },
        ...
        initialize: function() {
            ...
            this.initListeners();
            $('.order-addresses').after($('.order-view-cssoft-fields'));
        },
        ...
        saveFields: function(saveUrl) {
            var provider = registry.get(this.provider),
                data = provider.cssoftCheckoutFields;
        ...
        validateFields: function(provider) {
            ...
            provider.trigger('cssoftCheckoutFields.data.validate');
        ...
        scrollToFields: function() {
            $([document.documentElement, document.body]).animate({
                scrollTop: $(".order-view-cssoft-fields").offset().top - $('.page-actions._fixed').outerHeight()
            }, 500);
        }
    });
});
