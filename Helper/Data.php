<?php
namespace CSSoft\CheckoutDetails\Helper;

use Magento\Store\Model\ScopeInterface;
use CSSoft\CheckoutDetails\Api\Data\FieldDataInterfaceFactory;
use CSSoft\CheckoutDetails\Model\ResourceModel\Field\Option\CollectionFactory as OptionCollectionFactory;
...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @param \CSSoft\CheckoutDetails\Model\Field $field
     * @return string
     */
    ...
    public function getFieldComponent($field, $label, $validation, $default, $options, $provider)
    {
        $config = [
            ...
            'dataScope' => 'cssoftCheckoutFields.cssoft_checkout_field[' . $field->getAttributeCode() . ']',
            'label' => $label,
            'provider' => $provider,
            'visible' => true,
            'validation' => $validation,
            'sortOrder' => $field->getSortOrder(),
            'id' => 'cssoft_checkout_field[' . $field->getAttributeCode() . ']',
            'value' => $default
        ];
    ...
    protected function getComponentConfig($field)
    {
        $config = [
            ...
            'customScope' => 'cssoftCheckoutFields',
            'elementTmpl' => $this->getFieldTemplate($field),
            'multiple' => $field->getFrontendInput() == 'multiselect',
            'additionalClasses' => 'cssoft-checkout-fields__field',
            'cssoftCheckoutField' => true
        ];
    ...
    public function parseDateString($date, $locale = null)
    {
        return $this->localeDate->date($date, $locale);
    }
    ...
    public function getOrderFieldsValues($order, $selectedFields = null)
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($field->getFrontendInput() == 'date') {
                $value = new \DateTime($field->getValue());
                $field->setValue($this->getFormattedDate($value));
            } elseif ($field->getFrontendInput() == 'boolean') {
