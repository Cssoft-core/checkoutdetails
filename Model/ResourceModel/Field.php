<?php
namespace CSSoft\CheckoutDetails\Model\ResourceModel;
...
class Field extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('cssoft_checkoutdetails_field', 'field_id');
    }
     ...
     * @return \CSSoft\CheckoutDetails\Model\ResourceModel\Field
     */
    protected function _beforeDelete(AbstractModel $object)
    {
        $condition = ['field_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('cssoft_checkoutdetails_store'), $condition);
    ...
    protected function _afterSave(AbstractModel $object)
    {
        ...
        $table = $this->getTable('cssoft_checkoutdetails_store');
     ...
     * @param string $field
     * @param mixed $value
     * @param \CSSoft\CheckoutDetails\Model\Field $object
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['fts' => $this->getTable('cssoft_checkoutdetails_store')],
    ...
    public function lookupStoreIds($id)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('cssoft_checkoutdetails_store'),
            'store_id'
        )->where(
            'field_id = :field_id'
        );
    ...
    protected function saveStoreLabels(AbstractModel $object)
    {
        ...
        if (is_array($storeLabels)) {
            ...
            if ($object->getId()) {
                ...
                $connection->delete($this->getTable('cssoft_checkoutdetails_field_label'), $condition);
            }
            foreach ($storeLabels as $storeId => $label) {
                ...
                $connection->insert($this->getTable('cssoft_checkoutdetails_field_label'), $bind);
            }
        }
    ...
    {
        $connection = $this->getConnection();
        $table = $this->getTable('cssoft_checkoutdetails_field_option');
    ...
    {
        $connection = $this->getConnection();
        $table = $this->getTable('cssoft_checkoutdetails_field_option_value');
    ...
    public function getStoreLabelsByFieldId($fieldId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('cssoft_checkoutdetails_field_label'),
            ['store_id', 'value']
        )->where(
            'field_id = :field_id'
        );
